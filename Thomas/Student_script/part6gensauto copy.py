...
def create_polycost_and_calc(Gen_obj_list):
    Gen_list = [[G1,G1_cost], [G2,G2_cost], [G3,G3_cost], [G4,G4_cost], [G5,G5_cost], [G6,G6_cost], [G7,G7_cost], [G8,G8_cost]]
    pp.create_poly_cost(net, element=Gen_obj_list[0], et='gen',cp2_eur_per_mw2=0.001, cp1_eur_per_mw=-0.7, cp0_eur=32.25)
    pp.create_poly_cost(net, element=Gen_obj_list[1], et='gen',cp2_eur_per_mw2=4.622e-4, cp1_eur_per_mw=-0.3467, cp0_eur=25)
    pp.create_poly_cost(net, element=Gen_obj_list[2], et='gen',cp2_eur_per_mw2=5.625e-4, cp1_eur_per_mw=-0.45, cp0_eur=120)
    pp.create_poly_cost(net, element=Gen_obj_list[3], et='gen',cp2_eur_per_mw2=9.876e-5, cp1_eur_per_mw=-0.089, cp0_eur=60)
    pp.create_poly_cost(net, element=Gen_obj_list[4], et="gen", cp1_eur_per_mw=-0.28,cp0_eur=80,cp2_eur_per_mw2=2.8e-4)
    pp.create_poly_cost(net, element=Gen_obj_list[5], et="gen", cp1_eur_per_mw=-0.3124,cp0_eur=90.0025,cp2_eur_per_mw2=2.81e-4)
    pp.create_poly_cost(net, element=Gen_obj_list[6], et="gen", cp1_eur_per_mw=-0.034776,cp0_eur=24.9981,cp2_eur_per_mw2=3.024e-5)
    pp.create_poly_cost(net, element=Gen_obj_list[7], et="gen", cp1_eur_per_mw=-0.6666,cp0_eur=234.98,cp2_eur_per_mw2=5.555e-4)

    pp.runopp(net)
    Total_quadra_pow = 0
    Total_quadra_cost = 0

    for i,gen_number in enumerate(Gen_obj_list):
        wp = net.res_gen["p_mw"][gen_number]
        Total_quadra_pow += wp
        Total_quadra_cost += Gen_list[i][1](wp)

    
    return Total_quadra_cost

def find_better_cost():
    """Return a list where the index is the cost function of the inital generator and the value is the found one
    """
    Generator_list = [G1,G2,G3,G4,G5,G6,G7,G8]
    initial_cost = create_polycost_and_calc(Generator_list)
    actual_cost = 999999999999999999999999
    
    attempt = 0
    
    while actual_cost > initial_cost:
        print(f"Attempt {attempt} | initial {initial_cost} | actual {actual_cost}")
        random.shuffle(Generator_list)
        actual_cost = create_polycost_and_calc(Generator_list)
        attempt += 1
        
    print(f"Initial cost {initial_cost} | Found cost {actual_cost} in {attempt} attempts | Configuration {Generator_list}")
    return Generator_list, actual_cost
